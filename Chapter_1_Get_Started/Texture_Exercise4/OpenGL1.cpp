...
void processInput(GLFWwindow *window, Shader &shader)
{
	...
	{
		if (shader.mixRatio < 1.0f)
		{
			shader.mixRatio += 0.0005f;
			shader.setFloat("mixRatio", shader.mixRatio);
		}
	}
	else if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS)
	{
		if (shader.mixRatio > 0.0f)
		{
			shader.mixRatio -= 0.0005f;
			shader.setFloat("mixRatio", shader.mixRatio);
		}
...
int main()
{
	...
	ourShader.use();
	ourShader.setFloat("mixRatio", ourShader.mixRatio);
